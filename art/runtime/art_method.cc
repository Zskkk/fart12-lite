...
void ArtMethod::SetPackageItem(JNIEnv* env,jobject config){
    ...
    jclass jcInfo = env->FindClass("cn/zskkk/PackageItem");
    ...
    oss << "mikrom SetPackageItem isDeep:"<<packageConfig.isDeep;
    LOG(ERROR)<< oss.str();
}
...
extern "C" void dumpDexOver()  REQUIRES_SHARED(Locks::mutator_lock_) {
    ...
    for(iter = dex_map.begin(); iter != dex_map.end(); iter++) {
        ...
        sprintf(dexfilepath,"/data/data/%s/zskkk",packageName);
        mkdir(dexfilepath,0777);
        memset(dexfilepath,0,1000);
        sprintf(dexfilepath,"/data/data/%s/zskkk/%d_dexfile_repair.dex",packageName,size_int_);
...
extern "C" void dumpdexfilebyExecute(ArtMethod* artmethod)  REQUIRES_SHARED(Locks::mutator_lock_) {
    ...
    mkdir(dexfilepath,0777);
    memset(dexfilepath,0,1000);
    sprintf(dexfilepath,"/data/data/%s/zskkk/%d_dexfile_execute.dex",packageName,size_int_);
    int dexfilefp=open(dexfilepath,O_RDONLY,0666);
    if(dexfilefp>0){
      close(dexfilefp);
      dexfilefp=0;
              ...
              if(fp>0)
              {
                  ...
                  close(fp);
                  memset(dexfilepath,0,1000);
                  sprintf(dexfilepath,"/data/data/%s/zskkk/%d_classlist_execute.txt",packageName,size_int_);
...
extern "C" void dumpArtMethod(ArtMethod* artmethod)  REQUIRES_SHARED(Locks::mutator_lock_) {
			...
			char* packageName=ArtMethod::GetPackageName();
				...
				sprintf(dexfilepath,"/data/data/%s/zskkk/%d%s_dexfile.dex",packageName,size_int_,deepstr);
				int dexfilefp=open(dexfilepath,O_RDONLY,0666);
				if(dexfilefp>0){
					close(dexfilefp);
					dexfilefp=0;
					...
					if(fp>0)
					{
							...
							close(fp);
							memset(dexfilepath,0,1000);
							sprintf(dexfilepath,"/data/data/%s/zskkk/%d%s_classlist.txt",packageName,size_int_,deepstr);
							int classlistfile=open(dexfilepath,O_CREAT|O_APPEND|O_RDWR,0666);
							if(classlistfile>0)
							{
										for (size_t ii= 0; ii< dex_file->NumClassDefs(); ++ii)
				...
				if (LIKELY(code_item != nullptr))
				{
							...
							code_item_len = (int)artmethod->GetDexFile()->GetCodeItemSize(*code_item);
							memset(dexfilepath,0,1000);
							int size_int=(int)dex_file->Size();
							uint32_t method_idx=artmethod->GetDexMethodIndex();
							sprintf(dexfilepath,"/data/data/%s/zskkk/%d%s_ins_%d.bin",packageName,size_int,deepstr,(int)gettidv1());
